...
def main():
    ...
    graphs_dir = Path("curve25519-dalek_public_apis_graphs")
    ...
    for dot_file in dot_files:
        ...
        if grey_nodes or sink_node:
            print(f"  {dot_file.name}: found {len(grey_nodes)} grey nodes, sink: {sink_node[1] if sink_node else 'None'}")
    ...
    output = {
        ...
        "grey_nodes": [
            ...
            for file, function in grey_nodes_list
        ],
        "unique_sink_nodes": [
            {
                "file": file,
            ...
            for file, function in sink_nodes_list
        ],
        "all_sink_nodes": all_sink_nodes_per_file
    }
    
    # Write to JSON file
    output_file = "grey_nodes_extracted.json"
    ...
    duplicate_sinks = [(sink, files) for sink, files in sink_node_to_files.items() if len(files) > 1]
    if duplicate_sinks:
