...
def main():
    csv_path = Path(__file__).parent.parent.parent / "outputs" / "curve25519_functions.csv"
    md_path = Path(__file__).parent.parent.parent / "outputs" / "curve25519_functions.md"
    ...
    with open(csv_path, "r", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            has_spec_hax = row["has_spec_hax"].strip()
            has_proof_hax = row["has_proof_hax"].strip()
            ...
            spec_hax_cb = to_checkbox(has_spec_hax)
            proof_hax_cb = to_checkbox(has_proof_hax)

            lines.append(
                f"| {function_cell} | {spec_verus_cb} | {proof_verus_cb} | {spec_hax_cb} | {proof_hax_cb} |"
            )
